...
export const AdminOrdersScreen = () => {
  ...
  const {
    ...
    queryKey: ['adminOrders', statusFilter, page, user?._id || user?.id],
    queryFn: async () => {
      if (__DEV__) {
        console.log('🔍 Fetching admin orders with params:', {
    ...
    enabled: !!user && (user?.role === 'admin' || (user as any)?.role === 'ADMIN'),
  ...
  const { data: allOrdersResponse } = useQuery({
    queryKey: ['adminOrders', 'all', user?._id || user?.id],
    queryFn: async () => {
      const response = await orderApi.getAllOrders({
        page: 1,
